...
void SDD::add_synthesized_attribute(const CFG::production_type &production,
  ...
  if (!result_attribute_name.belong_to_nonterminal(production.first)) {
...
void SDD::add_inherited_attribute(const CFG::production_type &production,
                                  semantic_rule rule) {
  if (!cfg.has_production(production)) {
    throw exception::unexisted_production("");
  }
  ...
  auto const result_grammar_it =
      std::find_if(production.second.begin(), production.second.end(),
                   [&result_attribute_name](auto &grammar_symbol) {
                     return grammar_symbol.is_nonterminal() &&
                            result_attribute_name.belong_to_nonterminal(
                                *grammar_symbol.get_nonterminal_ptr());
                   });
  if (result_grammar_it == production.second.end()) {
    throw exception::unexisted_grammar_symbol_attribute(
        result_attribute_name.get_name());
  }
  ...
  for (auto const &argument : rule.arguments) {
    ...
    if (argument.belong_to_nonterminal(production.first)) {
